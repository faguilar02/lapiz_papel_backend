...
/**
 * Builder UBL 2.1 mejorado que usa el JSON completo de SUNAT
 * Incluye todos los campos requeridos y opcionales
 */
export class EnhancedUblBuilder {
  /**
   * Construye XML UBL 2.1 completo desde el JSON SUNAT
   */
  buildFromSunatJson(sunatJson: any): {
    ...
    // Convertir todos los valores monetarios a números
    const mtoOperGravadas = toNumber(_mtoOperGravadas);
    const mtoIGV = toNumber(_mtoIGV);
    const totalImpuestos = toNumber(_totalImpuestos);
    ...
    // Determinar si es Invoice o CreditNote/DebitNote
    ...
    const root = create({ version: '1.0', encoding: 'UTF-8' })
      ...
      // Tipo de documento con catálogo SUNAT
      .ele(
      ...
      // === EMISOR (SUPPLIER) === UBL 2.1 Perú correcto
      ...
      // === CLIENTE (CUSTOMER) === UBL 2.1 Perú correcto
      ...
      // Dirección del cliente (obligatorio para facturas)
      .ele('cac:RegistrationAddress')
      .ele('cac:AddressLine')
      .ele('cbc:Line')
    ...
    details.forEach((item: any, index: number) => {
      // Convertir valores numéricos del item de forma segura
      ...
      root
        ...
        .txt('01')
        .up() // Precio unitario (incluye el IGV)
        .up()
        .up()
        ...
        .txt('50000000')
        .up() // Código genérico UNSPSC
        .up()
        .up()

        // Precio unitario (valor de venta sin IGV)
        .ele('cac:Price')
        .ele('cbc:PriceAmount', { currencyID: tipoMoneda })
